...
# OCR noise: stray punctuation and digits dropped between letters. We strip
# any token that consists ONLY of these characters before re-joining.
_NOISE_TOKEN = re.compile(r"^[!|@#%^&*+=~?,;:_/\\\d]+$")


def _denoise(s: str) -> str:
    """Drop pure-noise tokens out of a multi-word OCR fragment.

    "Partrdge ! Ave" → "Partrdge Ave". We only delete tokens that have *no*
    letters at all, so a typo'd word like "Brocmfield" survives intact for
    a downstream Levenshtein-style match.
    """
    keep = [tok for tok in s.split() if not _NOISE_TOKEN.match(tok)]
    return " ".join(keep)
...
def looks_like_street(text: str) -> bool:
    ...
    s = _denoise(_collapse_ws(_strip_quotes(text)))
...
# (regex, replacement) substitutions applied at every match position. Each
# match site produces one variant — small alphabet, won't blow up quickly.
_SUBSTITUTIONS: tuple[tuple[re.Pattern, str], ...] = (
    (re.compile(r"cm"), "om"),                # 'o' read as 'c'
    (re.compile(r"om"), "cm"),                # the reverse
    (re.compile(r"rn"), "m"),                 # two strokes ↔ single arch
    (re.compile(r"m"),  "rn"),
    (re.compile(r"I"),  "l"),                 # capital I / lowercase l
    (re.compile(r"l"),  "i"),                 # lowercase l → i (sans-serif)
    (re.compile(rf"([{_CONSONANTS}])i([{_CONSONANTS}])", re.IGNORECASE),
     r"\1\2"),                                # drop stray 'i' between consonants
)

# Insert 'i' between every adjacent consonant pair — recovers vowel drops
# such as "Partrdge → Partridge". Each adjacent pair is one variant.
_INSERT_VOWEL = re.compile(rf"(?=[{_CONSONANTS}][{_CONSONANTS}])", re.IGNORECASE)


def _gen_substitutions(name: str) -> list[str]:
    """One substitution-variant per (rule, match position)."""
    out: list[str] = []
    for pattern, repl in _SUBSTITUTIONS:
        for m in pattern.finditer(name):
            new = name[: m.start()] + pattern.sub(repl, m.group(0), count=1) + name[m.end():]
            if new != name:
                out.append(new)
    return out


def _gen_vowel_inserts(name: str) -> list[str]:
    """Insert 'i' between each adjacent consonant pair (one variant each)."""
    out: list[str] = []
    for m in _INSERT_VOWEL.finditer(name):
        # m.start() is the index BEFORE the second consonant.
        idx = m.start()
        out.append(name[:idx] + "i" + name[idx:])
    return out


def name_variants(name: str, *, max_variants: int = 6) -> list[str]:
    """Generate up to ``max_variants`` plausible OCR-typo variants of ``name``.

    The original is always first. Variants are generated by:
      1. Single-character substitutions at common OCR-confusion sites
         (cm↔om, rn↔m, l↔I↔i, dropped 'i' between consonants).
      2. Inserting 'i' between adjacent consonant pairs (recovers vowel drops
         like "Partrdge → Partridge").

    Used by the cross-reference layer to widen the Nominatim query —
    Nominatim's full-text matcher is not typo-tolerant, so without these
    variants "Brocmfield Road" returns zero hits even though
    "Broomfield Road" is well-indexed.
    """
    seen: dict[str, None] = {name: None}
    for v in _gen_substitutions(name):
        if v not in seen:
            seen[v] = None
            if len(seen) - 1 >= max_variants:
                return list(seen)
    for v in _gen_vowel_inserts(name):
        if v not in seen:
            seen[v] = None
            if len(seen) - 1 >= max_variants:
                return list(seen)
